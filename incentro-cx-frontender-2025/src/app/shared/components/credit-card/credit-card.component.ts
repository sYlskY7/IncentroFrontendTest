  ...
  <div class="card-3d w-full h-full" [style.transform]="isFlipped ? 'rotateY(180deg)' : 'rotateY(0deg)'">
    ...
    <div class="absolute inset-0 bg-paper rounded-card shadow-card p-6 flex flex-col justify-between face">
      ...
      <div>
        ...
        <button type="button" (click)="toggleNumber($event)" class="mt-2 inline-flex items-center gap-1 text-xs text-blue-600 hover:opacity-80" aria-label="Toggle card number visibility">
    ...
    <div class="absolute inset-0 rounded-card shadow-card p-6 face bg-[#f4c144] text-gray-900" [style.transform]="'rotateY(180deg)'">
      ...
      <!-- Signature panel with CVV at the right -->
